...
class Dataset:
    ...
    def get_gt_infos(self, coords, labels, instances, bg_class=-100):
        # coords = points[:, :3]
        # labels = points[:, 6]
        # instances = points[:, 7]
        fg_inds = (instances != bg_class) & (labels != bg_class)
        instance_ids = np.unique(instances[fg_inds])
        num_inst = instance_ids.shape[0]
        gt_boxes = np.zeros((num_inst, 6), dtype=np.float32)
        gt_spheres = np.zeros((num_inst, 4), dtype=np.float32)
        gt_labels = np.zeros(num_inst, dtype=np.long)
        gt_masks = np.zeros((num_inst, instances.shape[0]), dtype=np.uint8)
        for i, id in enumerate(instance_ids):
            pos_inds = (instances == id)
            gt_masks[i] = pos_inds
            _coords = coords[pos_inds]
            gt_boxes[i, :3] = _coords.min(axis=0)
            gt_boxes[i, 3:] = _coords.max(axis=0)
            centroid = _coords.mean(axis=0)
            radius = np.linalg.norm(_coords - centroid, axis=1).max()
            gt_spheres[i, :3] = centroid
            gt_spheres[i, 3] = radius
            gt_label_loc = np.nonzero(pos_inds)[0][0]
            gt_labels[i] = labels[gt_label_loc] - 2  # we ignore 2 first classes
        shape = np.concatenate([coords.min(axis=0), coords.max(axis=0)])
        gt_infos = dict(labels=gt_labels, boxes=gt_boxes, gt_spheres=gt_spheres,
                        masks=gt_masks, shape=shape, coords=coords)
        for k, v in gt_infos.items():
            gt_infos[k] = torch.from_numpy(v)
        return gt_infos

    def encode_labels(self, labels):
        # temporary use for sem seg
        inds = (labels == -100)
        labels[inds] = 20
        return labels
    ...
    def trainMerge(self, id):
        ...
        for i, idx in enumerate(id):
            ...
            # get instance level infos
            gt_info = self.get_gt_infos(xyz, label, instance_label)
            ...
            coord_feats.append(torch.from_numpy(xyz_middle))
        ...
        coord_feats = torch.cat(coord_feats, 0).float()          # float (N, 3)
        ...
        return {'locs': locs, 'voxel_locs': voxel_locs, 'p2v_map': p2v_map, 'v2p_map': v2p_map,
                'locs_float': locs_float, 'feats': feats, 'labels': self.encode_labels(labels), 'instance_labels': instance_labels,
                'instance_info': instance_infos, 'instance_pointnum': instance_pointnum,
                'id': id, 'offsets': batch_offsets, 'spatial_shape': spatial_shape, 'gt_infos': gt_infos, 'coord_feats': coord_feats}
