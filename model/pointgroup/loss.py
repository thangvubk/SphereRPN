...
class SetCriterion(nn.Module):
    ...
    def loss_clusters(self, outputs, targets, indices, num_boxes):
        assert 'pred_cluster' in outputs
        src_idx = self._get_src_permutation_idx(indices)
        tgt_idx = self._get_tgt_permutation_idx(indices)

        src_cluster = outputs['pred_cluster'][src_idx]
        tgt_cluster = []
        for t in targets:
        ...
        tgt_cluster = torch.cat([t[j] for t, (_, j) in zip(tgt_cluster, indices)])
        losses = {}
        losses['loss_cluster'] = F.l1_loss(src_cluster, tgt_cluster, reduction='sum') / num_boxes
        return losses

    def loss_masks1(self, outputs, targets, indices, num_boxes):
